...
sum.mi.mean.cont.simp <- sum.mi.mean.cont.simp%>%
  ...
  mutate(txtmean = sprintf("%.3f (%.3f)%s", estimate,  std.error, stars.pval(p.value)),
         var = str_remove(str_remove(rownames(sum.mi.mean.cont.simp), "_discrete"), "TRUE"))%>%
...
```{r GEEMImontone, echo=FALSE}
d6b %>% as.data.frame() %>%
  kable(booktabs = TRUE,
        row.names = FALSE,
        caption = "Model results for different imputation methods with monotone imputation.") %>%
  add_header_above(c(" ","MI GEE Mean MON" = 2, "MI GEE Norm MON" = 2, 
                     "MI GEE Pmm MON" = 2, "MI GEE Rf MON" = 2))
```
...
d6c %>% as.data.frame() %>%
  ...
  add_header_above(c(" ","MI GEE Mean FCS" = 2, "MI GEE Norm FCS" = 2, 
                     "MI GEE Pmm FCS" = 2, "MI GEE Rf FCS" = 2))
```
