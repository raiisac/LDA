...
sum.mi.mean.cont.simp <- sum.mi.mean.cont.simp%>%
  ...
  mutate(txtmean = sprintf("%.3f (%.3f)%s", estimate,  std.error, stars.pval(p.value)),
         var = rownames(sum.mi.mean.cont.simp))%>%
...
sum.mi.mean.cont.simp %>%
  ...
  kable(booktabs = TRUE,
        ...
        row.names = FALSE) %>%
  add_header_above(c("", "Mean imputation", "Random forest"))
...
It is likely that different assumption on the missingness mechanism will lead to different conclusions, especially if the dropout is higher than 10% [@10.3389/fpsyg.2017.00722].
