...
Figure \@ref(fig:missingtessovertime) clearly shows that subject rarely are measured two years in a row, most are not measured at $t=1$, and the number of subjects that stay missing gradually increases as time passes.
...
```{r missingtessovertime, include=TRUE, echo = FALSE, message=FALSE, warning=FALSE, fig.cap = "Number of subjects the are present, return, drop out or are missing at each time instance.", fig.height = 4, fig.width=6}
...
\left\{
                \begin{array}{ll}
                logit(R_{it}) = \beta_0 + \beta_1 TIME_{it} + \beta_3 side_{it} + \beta_4 age_{it} + b_i \\
                b_i \sim N(0,\sigma^2)
                \end{array}
          \right.
(\#eq:missingness)
...
The variable *age* was standardized to get convergence in the model. Table \@ref(tab:missing_mixedmodel) shows that the only truely significant variable is TIME; as time increases, subjects are more likely to be missing. We can therefore assume missingness at random (MAR).

```{r missing_mixedmodel, include=TRUE, echo = FALSE, message=FALSE, warning=FALSE, eval=FALSE}
library(lme4)
data_grid <- data_grid %>%mutate(
  R_num = 1*(R==TRUE),
  age_scale = scale(age, center = TRUE, scale = TRUE)
)
m1 <- glmer(formula = R_num ~ TIME_discrete  + side + age_scale + (1|id),
        data = data_grid,
        family=binomial(link = "logit"))
s <- summary(m1) 
s$coefficients %>% as.data.frame() %>%
  mutate(var = c('Intercept', "TIME", "sideright","age"),
