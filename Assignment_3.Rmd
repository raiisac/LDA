...
```{r resultsintercept, echo=FALSE, fig.cap="Sensitivity analysis of the effect on the estimated intercept. The rows shows different values for deltage. The columns show different values for deltatime.", fig.width = 8}
...
```{r resultssens,echo=FALSE, fig.cap="Sensitivity analysis of the effect on all parameter estimates, except for the intercept. The rows shows different values for deltage. The columns show different values for deltatime." , fig.width = 8}
results %>% filter(term != "(Intercept)") %>%
  mutate(term = str_remove(term, "_discrete")) %>%
  ggplot() +
  geom_line(aes(x=intercept, y = estimate, color = term)) +
