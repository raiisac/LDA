...
Using what we have learned from the previous sections, and after test several models for the mean structure, we end up with the model $Y_{ijk} = age_{ij} + age^2_{ij} + learning_{ijk} + TIME_{ijk} + age_{ij} * TIME_{ijk}$ with a random slope and random intercept with $id$ and $side$ as grouping variables (although there was no overall population effect of $side$, it appears that some peoples' left ear consistently scores better/worse than their right ear).
...
dat_mixed5 <- data.frame(time = data$TIME,
                       ...
                       Subject = data$id,
                       side = data$side) %>%
  left_join(data[,c("age", "age_discrete","id")], by = c("Subject" = "id"))
...
ggplot(data = dat_mixed5, aes(x = time, y = pred, color = age, 
  ...
  ylab ("Hearing threshold") + facet_wrap(facets = vars(age_discrete))
```
...
We observe that on average men’s hearing degrades as they get older, especially after 50 years of age. Men's hearing thresholds increase over time from the age of about 20 years old and the speed of hearing loss increases as the subjects get older. We do not see any difference between the left and right ear.
